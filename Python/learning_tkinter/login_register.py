...
def file_append():
    ...
    for obj in register["Register"]:
        obj["name"] = e_name.get()
        obj["last_name"] = e_lastname.get()
        obj["email"] = e_email.get()
        obj["password"] = e_password.get()
        obj["confirm_password"] = e_confpassword.get()
...
def place():
    ...
    e_mail = Entry(root_2, width=25, bg="#f2f1f1")
    e_pass = Entry(root_2, width=25, bg="#f2f1f1")
    ...
    e_mail.grid(row=1, column=1, ipady=7, pady=(40,20), padx=(115,0))
    e_pass.grid(row=2, column=1, ipady=7, pady=(20,40), padx=(115,0))       
...
def find_log():
    with open("register.json", "r") as f:
        ...
        for i in f_content:
            if i == e_mail.get() and e_pass.get():
